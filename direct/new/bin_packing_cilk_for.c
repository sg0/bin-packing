...
#if defined(USE_PTHREAD_MTX)
#include <pthread.h>
#endif
// else use Intel atomic intrinsics
...
#if defined(USE_PTHREAD_MTX)
void search_bin_for(int item, pthread_mutex_t *mtx)
{
    int abort = 0;
    item_in_bin[item] = 1;
    for (int i = 0; ((i < max_num_bins) && !abort); i++)
    {
	// no atomic support in cilk plus
...
#else
void search_bin_for(int item)
{
    int abort = 0;
    item_in_bin[item] = 1;
    for (int i = 0; ((i < max_num_bins) && !abort); i++)
    {
	__sync_sub_and_fetch(&space_left_on_bin[i], item);
	...
	{
	    __sync_add_and_fetch(&space_left_on_bin[i], item);
	}
    }
}
#endif
...
{
#if defined(USE_PTHREAD_MTX)
    // init lock for atomics
    pthread_mutex_t m; //define the lock
    pthread_mutex_init(&m, NULL); //initialize the lock
#endif    
    cilk_for(int u = 0; u < num_items; u++)
    {
	// cilk task spawn
#if defined(USE_PTHREAD_MTX)
	search_bin_for(items[u], &m);
#else
	search_bin_for(items[u]);
#endif
    }
}
...
main (int argc, char *argv[])
{
  ...
  // control number of workers via CILK_NWORKERS
  // environment var
  ...
  time_inserts = ( stop.tv_sec - start.tv_sec )
  ...
  printf
    ("\n Maximum number of bins = %d\n Maximum items per bin = %d\n Time taken = %lf secs\n",
     max_num_bins, max_items_per_bin, time_inserts);
