...
#pragma omp parallel for \
    ...
    {
	int item = items[u];
	// seedp should be in different address for
	// each thread
	unsigned int seedi = omp_get_thread_num () + item;
	// generate i and j to randomly determine a bin cell
	int i = ((int) rand_r (&seedi)) % max_num_bins;
	int abort = 0;
...
main (int argc, char *argv[])
{
  ...
  while (1)			// iterate till all items have been inserted into m bins
    {
      // 1. insert items in bins
      insert_items (items, item_count);
      // reset before next iteration
      //memset (items, 0, item_count * sizeof (int));
      //item_count = 0;
      // 2. check whether the insertion was correct
      item_count = check_and_correct (items);

      // termination
      if (item_count == 0 || iters == MAX_ITERS)
	break;
  ...
  printf
    ("\n Maximum number of bins = %d\n Maximum items per bin = %d\n Iterations = %d\n Time taken = %f secs\n",
     max_num_bins, max_items_per_bin, iters, (end_time - start_time));
  ...
  if (check || iters == MAX_ITERS || nelems != m)
